...
5) On success, `OrderRequest` is created from the stored `orderDraft`
   - `fulfillmentStatus` transitions to `SUCCESS` when creation completes
   - Amount is validated against `amountToman * 10` from Zibal verification
6) Frontend polls `GET /payments/:id` or uses redirect params

## Fulfillment Retry (Admin)
- `POST /admin/photo-restore/payments/:id/fulfill` (JWT + `admin.finance:manage`)
- Use when payment is `SUCCESS` but `fulfillmentStatus` is not `SUCCESS`.
