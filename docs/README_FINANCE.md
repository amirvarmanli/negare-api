...
6. On success, order is marked PAID, entitlements are granted, and revenue splits are recorded with supplier/platform percentages (default 70/30).
   ...
   - `fulfillmentStatus` is recorded on the payment (`SUCCESS` once downstream steps complete).
...
### Donation (wallet)
- `POST /donations/pay-with-wallet` debits the wallet and records a donation payment with fulfillment tracking.

### Fulfillment Retry (Admin)
- `POST /admin/payments/:id/fulfill` retries downstream fulfillment when a payment is `SUCCESS` but not fulfilled.
...
1. `GET /subscriptions/plans` to fetch active plans from `finance.subscription_plans_v2`.
2. `POST /subscriptions/purchase` with `{ planId }` to create a pending subscription purchase.
3. Pay via `POST /payments/start` with `{ refType: "subscription", refId: purchaseId }`
   or wallet via `POST /wallet/pay` with the same reference.
4. On successful callback/verification, the purchase becomes `PAID` and the user
   subscription is created or extended based on `plan.durationDays`.
...
- Run Prisma migrations:
  ...
  - Ensure `finance.subscription_purchases.subscription_plan_id` exists (migration
